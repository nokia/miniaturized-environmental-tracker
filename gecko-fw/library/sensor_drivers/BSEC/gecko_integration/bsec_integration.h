...
// Debug option - use the infinite loop version to check that all is working properly. 
// Should not be defined for normal use.
#define BSEC_LOOP 0
...
typedef struct {
  uint32_t pressure;          //  BME680 pressure; 
  uint32_t temperature;       //  BME680 temperature; 
  uint32_t humidity;          //  BME680 humidity; 
  uint32_t gas_resistance;    //  BME680 gas_resistance; 
  uint32_t gas_quality;       //  BSEC Gas Quality
} t_bsec_data;

/* Callback to the main function to transfer data. */
//typedef void (*bsec_callback_fn)(t_bsec_data*);

typedef void (*bsec_callback_fn)(int64_t timestamp, float iaq, uint8_t iaq_accuracy, float temperature, float humidity,
...
#if BSEC_LOOP == 1
void bsec_iot_loop(void * p_context);
#else
...
void bsec_iot_sensor_control_handler(void * p_context);
void bsec_iot_read_process_handler(void * p_context);

// Pointer to context to pass between app_timer handlers.
typedef struct {
  sleep_fct             sleep;
  get_timestamp_us_fct  get_timestamp_us;
  output_ready_fct      output_ready;
  state_save_fct        state_save;
  bsec_callback_fn      bsec_callback;
